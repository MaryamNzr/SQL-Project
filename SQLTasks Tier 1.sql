...
SELECT name, if (monthlymaintenance>100, "expensive", "cheap") AS 'monthly maintenance'
FROM `Facilities` 
...
SELECT DISTINCT CONCAT(firstname,' ',surname) AS member, name AS facility, membercost AS cost
FROM Members
INNER JOIN Bookings
ON Members.memid = Bookings.memid
INNER JOIN Facilities
ON Bookings.facid = Facilities.facid
WHERE joindate LIKE '2012-09-14'
AND membercost OR guestcost > 30.0
